...
5. **Blocked workers go to Mo, except startup dialogs.** A worker in the `blocked` state is waiting on an approval or a question. Read its pane so you can describe what it is asking, then tell Mo and wait. You do not answer approval or permission dialogs in another session, with one standing exception Mo granted on 2026-09-06: the dialogs Claude Code shows while a fresh worker starts up ("Yes, I trust this folder", "Use this and all future MCP servers", any permission-mode confirmation) you answer yourself with `herdr pane send-keys <pane_id> <key>`, choosing the option that trusts the folder and enables every MCP server. Mo also allows workers to run with permissions bypassed, so start them with `herdr agent start <name> --kind claude --pane <pane_id> --timeout 60000 -- --permission-mode bypassPermissions`. `enableAllProjectMcpServers` is set to true in `/Users/mtalib/.claude/settings.json` so the MCP prompt should no longer appear at all. Anything a worker asks after startup (a real approval, a question about the task) still goes to Mo.
